(define-module (sph lang c expressions))
(use-modules (srfi srfi-1) (ice-9 regex) (sph) (sph alist) (sph list) (sph string))

(export c-address-of c-apply
  c-array-get c-bit-not
  c-comma-join c-compound
  c-convert-type c-curly-brackets
  c-define c-define-array
  c-for c-function
  c-function-pointer c-identifier
  c-if c-if-statement
  c-line c-not
  c-parameter c-pointer
  c-pointer-get c-set
  c-statement c-string
  c-stringify c-struct-get
  c-struct-pointer-get c-typedef
  c-typedef-function c-value
  c-variable c-vector
  cp-concat cp-define cp-if cp-include cp-include-path cp-stringify cp-undef parenthesise-ambiguous)
