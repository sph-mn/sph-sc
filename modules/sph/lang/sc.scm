...
(use-modules (ice-9 match) (sph)
  ...
  (sph lang c expressions) (sph lang sc expressions) (sph lang sc check))

(re-export sc-syntax-check sc-syntax-error sc-syntax-error?)
(export sc->c sc-default-load-paths sph-lang-sc-description)
      ...
      (string-trim
        (regexp-replace
          (tree-transform (sc->sc a state)
            (l (a compile)
              (let* ((prefix (first a)) (a (tail a)) (b (descend-expr->c prefix a compile state)))
                (if b (list b #f) (list #f #t))))
            identity sc-value)
          "\n\n+" "\n")))))
