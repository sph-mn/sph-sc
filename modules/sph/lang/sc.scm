        ...
        (l (e)
          (match e
            ( (name type ... (? integer? bits))
              (string-append (string-join (map sc-identifier type) " " (q suffix))
    ...
    (case (first a)
      ( (set)
        (match (tail a)
          ...
          ((name value) (c-set-nc (compile name) (compile value)))))
      ( (define)
        (match (tail a)
          ( ( (name parameter ...) (return-type types ...) body ...)
            (c-function-nc (sc-identifier (compile name)) (sc-identifier return-type)
              (if (null? body) #f (join-expressions (map compile body)))
              (c-parameter (map sc-identifier parameter) (map sc-identifier types))))
          ( ( (name parameter ...) return-type body ...)
            (c-function (sc-identifier (compile name)) (sc-identifier return-type)
              (if (null? body) #f (join-expressions (map compile body)))))
          ( (name-1 type-1 name-2 type-2 rest ...)
            (join-expressions (map-slice 2 sc-define (tail a))))
          ( (name type value ...)
            (c-define-nc (sc-identifier name) (sc-identifier type)
      ...
      ( (define-macro)
        (match (tail a)
          ...
          ( (name-1 value-1 name-2 value-2 rest ...)
            (string-join
              (map-slice 2
                (l (name value) (cp-define-macro-nc (sc-identifier name) (compile value) #f))
                (tail a))
              "\n"))
          ( (name body ...)
            (cp-define-macro-nc (sc-identifier name)
        ...
        (match (tail a)
          ( (test body ...)
            (string-append "while" (parenthesise (compile test))
              (c-compound-nc (compile (pair (q begin) body)))))))
      ( (do-while)
        (match (tail a)
          ( (test body ...)
      ...
      ( (let-macro)
        ...
        (match (tail a)
          ( ( (names+values ...) body ...)
            (string-append
              (string-join
                (map-slice 2 (l (n v) (compile (list (q define-macro) n v))) names+values) "\n"
                (q suffix))
