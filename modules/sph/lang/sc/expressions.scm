(define-module (sph lang sc expressions))

(use-modules (srfi srfi-1) (ice-9 match)
  (ice-9 regex) (rnrs io simple)
  (sph) (sph alist)
  (sph filesystem) (sph hashtable)
  (sph lang c expressions) (sph lang scheme)
  (sph list) (sph string)
  ((sph tree) #:select (tree-contains?)) ((sph vector) #:select (vector-accessor)))

(export add-begin-if-multiple contains-set?
  not-function-pointer-symbol? not-preprocessor-keyword?
  preprocessor-keyword? sc-apply
  sc-case sc-compile-type
  sc-compile-types sc-cond
  sc-cond* sc-declare
  sc-define sc-define-array
  sc-define-function sc-define-type
  sc-do-while sc-enum
  sc-for sc-function
  sc-function-pointer sc-function-pointer?
  sc-identifier sc-identifier-list
  sc-if sc-if*
  sc-include-sc sc-include-sc-once
  sc-join-expressions sc-let*
  sc-macro-function sc-no-semicolon
  sc-numeric-boolean sc-path->full-path
  sc-pre-cond sc-pre-define
  sc-pre-define-if-not-defined sc-pre-define-multiple
  sc-pre-if sc-pre-include
  sc-pre-include-define sc-pre-include-variable
  sc-pre-let* sc-set
  sc-set-multiple sc-state-load-paths
  sc-state-new sc-state-no-semicolon
  sc-struct-literal sc-struct-or-union sc-struct-or-union-body sc-value sc-while)
