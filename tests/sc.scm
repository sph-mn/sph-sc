...
(execute-tests-quasiquote
  (sc->c
    ...
    "((a==3u)?((b_and_c=4u),myproc(a,b_and_c)):a)"
    (if (= a 3) (exit 1) (return (bit-or b c)))
    "if((a==3u)){exit(1u);}else{return((b|c));}"
    (set a 1 b+2 2 c-3 3)
    "a=1u;b_and_2=2u;c_3=3u;"
    (if* (not a) #t #f) "(!a?1u:0u)"
    (define a b32 1) "b32 a=1u"
    (define a b32 b+2 b16 c-3 b0) "b32 a;b16 b_and_2;b0 c_3;"
    (set a 1) "a=1u"
    (deref a 1) "*(a+1u)"
    (deref (deref a 1) 2) "*(*(a+1u)+2u)"
    (not 1) "!1u"
    (if* (not 1) a b) "(!1u?a:b)"
    (and 1 2 3) "(1u&&2u&&3u)"
    (= 1 2 3) "(1u==2u==3u)"
    ...
    (cond*
      ...
      ((equal? c 3) #f #t)
      (else #t #f))
    "((a==1u)?(b==2u):((c==3u)?(0u,1u):(1u,0u)))"
    (quote "var a = 3")
    "var a = 3"
    (return) "return"
    (return 1 2) "return(1u,2u)"
    (if 1 2 (begin 3 4 (return #t)))
    "if(1u){2u;}else{3u;4u;return(1u);}"
    (deref a-b)
    "(*a_b)"
    (struct-ref a b)
    "a.b"
    (struct-ref (deref a) b)
    "(*a).b"
    (define-macro (my-macro a b) (if* a #t #f))
    "#define my_macro(a,b) (a?1u:0u)"
    (define-macro ob-ject 3)
    "#define ob_ject 3u"
    (pre-if (equal? a b)
      (begin c d e)
      (begin f g))
    ...
    (struct (function-pointer a b c d) (b int))
    "struct{b(*a)(c,d);int b;}"
    (union (a unsigned int) (b unsigned char 3))
    "union{unsigned int a;unsigned char b:3u;}"
    (function-pointer f int char size_t)
    "int(*f)(char,size_t)"
