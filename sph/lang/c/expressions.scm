(library (sph lang c expressions)
  ...
  (define (cp-define-macro-nc name body identifier-list)
    (string-append
      "#define " name (if identifier-list identifier-list "") " " body))
    ...
    (string-append "#"
      (if (eq? (q if) type) "if"
        (if (eq? (q ifdef) type) "ifdef"
          (if (eq? (q ifndef) type) "ifndef"
            (raise (q cannot-convert-to-c)))))
      " " test "\n" consequent "\n"
      (if alternate
        (string-append "#else\n" alternate "\n")
        "")
  ...
  (define* (c-statement-nc keyword body #:optional arg)
    (string-append keyword
      (if arg (parenthesise arg) "")
      (c-compound-nc body)))
      ...
      (c-compound-nc
        (string-join
          (map
            (l (ele)
              (if (list? ele)
                (string-append (first ele) "=" (first (tail ele)))
                ele))
            enum-list)
          ","))))
  ...
  (define (c-identifier-list arg)
    (parenthesise
      (if (list? arg) (string-join (map c-identifier arg) ",")
        (if (or (symbol? arg) (string? arg))
          (c-identifier arg)
          (raise (q cannot-convert-to-c-identifier))))))

  (define (c-identifier+type arg type)
    (string-append (c-identifier type) " " (c-identifier arg)))

  (define (c-parameter arg types)
    (parenthesise
      (if (list? arg)
        (string-join (map c-identifier+type arg types) ",")
        (if (or (symbol? arg) (string? arg))
          (c-identifier+type arg types)
  ...
  (define* (c-apply-nc proc-name #:optional (args ""))
    (string-append proc-name "(" args ")"))

  (define* (c-apply proc-name #:optional (args (list)))
    (c-apply-nc (c-identifier proc-name) (string-join (map c-value args) ",")))

  (define* (c-if test consequent #:optional alternate) "string string [string] -> string
    create an if expression"
    (string-append "(" test "?" consequent ":" (if alternate alternate "0") ")"))

  (define* (c-if-statement test consequent #:optional alternate) "string string [string] -> string
    create an if statement"
    (string-append
      "if(" test "){" consequent "}"
      (if alternate
        (string-append "else{" alternate "}")
        "")))

  (define (c-pointer-deref-nc arg key)
    (if key
      (string-append "*(" arg "+" key ")")
      (string-append "(*" arg ")")))

  (define (c-bit-shift-nc left? arg shift-by)
    (string-append arg (if left? "<<" ">>") shift-by))
  ...
  (define (c-struct-ref arg key)
    (c-struct-ref-nc (c-identifier arg) (c-identifier key)))
  ...
  (define (c-string str)
    (string-enclose
      (fold
        (l (ele res)
          (string-replace-string res (first ele) (tail ele)))
        str c-escape-single-char)
      "\""))

  (define (c-value arg)
    "handles the default conversions between scheme and c types"
    (cond
      ((symbol? arg) (symbol->string arg))
      ((string? arg) (c-string arg))
      ((number? arg) (number->string arg))
      ((boolean? arg) (if arg "1" "0"))
      ((char? arg) (string-enclose (any->string arg) "'"))
      (else (raise (q cannot-convert-to-c))))))